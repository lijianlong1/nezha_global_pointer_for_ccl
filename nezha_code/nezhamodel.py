# Coding by long
# Datatime:2022/4/7 22:32
# Filename:nezhamodel.py
# Toolby: PyCharm
# description:
...
class NeZhaTcnModel(NeZhaPreTrainedModel):
    """
    The model can behave as an encoder (with only self-attention) as well
    as a decoder, in which case a layer of cross-attention is added between
    the self-attention layers, following the architecture described in `Attention is all you need`_ by Ashish Vaswani,
    ...
    Examples::
        ...
        sequence_output = encoder_outputs[0]
        # 在这里加上相关的tcn模块进行模型的预训练
        sequence_output = sequence_output.transpose(1, 2)
        # 将数据处理成为适合送入TCN模块的格式化数据
        sequence_output = self.tcn(sequence_output)  # 增加双层的
        # 直接加了一个两层的膨胀时空卷积
        sequence_output = sequence_output.transpose(1, 2)
...
class NeZhaTcnLstmModel(NeZhaPreTrainedModel):
    """
    The model can behave as an encoder (with only self-attention) as well
    as a decoder, in which case a layer of cross-attention is added between
    the self-attention layers, following the architecture described in `Attention is all you need`_ by Ashish Vaswani,
    ...
    Examples::
        ...
        sequence_output_nezha = encoder_outputs[0]
        # 在这里加上相关的tcn模块进行模型的预训练
        sequence_output_tcn = sequence_output_nezha.transpose(1, 2)
        # 将数据处理成为适合送入TCN模块的格式化数据
        sequence_output_tcn = self.tcn(sequence_output_tcn)  # 增加双层的
        # 直接加了一个两层的膨胀时空卷积
        sequence_output_tcn = sequence_output_tcn.transpose(1, 2)

        sequence_output_lstm, (_, _) = self.lstm(encoder_outputs[0])

        sequence_output = sequence_output_lstm + sequence_output_tcn
        # 直接使用加性融合算法，将后面两层的输出进行叠加融合
...
@add_start_docstrings("""Bert Model with a `language modeling` head on top. """, BERT_START_DOCSTRING)
class NeZhaTcnForMaskedLM(NeZhaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.bert = NeZhaModel(config)
        self.cls = BertOnlyMLMHead(config)
